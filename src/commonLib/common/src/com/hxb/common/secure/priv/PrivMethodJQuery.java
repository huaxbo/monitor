			...
			if(methods != null){
				for(Method m : methods){
					if(methodName.equals(m.toGenericString())){
						...
						String privs_value = ((CheckPrivJQuery)(m.getAnnotation(CheckPrivJQuery.class))).value() ;
						
						proceed = help.checkValue(m, powers , privs_value) ;
						if(!proceed){
							String[] privs_valueAll = ((CheckPrivJQuery)(m.getAnnotation(CheckPrivJQuery.class))).ifAll() ;
							proceed = help.checkIfAll(m, powers , privs_valueAll) ;
						}
						if(!proceed){
							String[] privs_valueAny = ((CheckPrivJQuery)(m.getAnnotation(CheckPrivJQuery.class))).ifAny() ;

			...
			if(methods != null){
				for(Method m : methods){
					if(methodName.equals(m.toGenericString())){
						...
						String privs_value = ((CheckPrivFlex)(m.getAnnotation(CheckPrivFlex.class))).value() ;
						
						proceed = help.checkValue(m, powers , privs_value) ;
						if(!proceed){
							String[] privs_valueAll = ((CheckPrivFlex)(m.getAnnotation(CheckPrivFlex.class))).ifAll() ;
							proceed = help.checkIfAll(m, powers , privs_valueAll) ;
						}
						if(!proceed){
							String[] privs_valueAny = ((CheckPrivFlex)(m.getAnnotation(CheckPrivFlex.class))).ifAny() ;

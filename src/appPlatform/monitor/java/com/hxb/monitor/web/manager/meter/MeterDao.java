...
@Repository
public class MeterDao extends HibernateAnnoBaseDao<TbCsMeter>{
					...
					public List<Object> doInHibernate(Session s) throws HibernateException,
						...
						try{
							...
							for(int i=0;i<lt.size();i++){
								...
								vo.build(po);
								TbCsServer ser = po.getServer();
								if(ser != null){
									vo.setServerNm(ser.getName());
								}
								list.add(vo);
							}
		...
		this.getHibernateTemplate().execute(
				new HibernateCallback() {					
					...
					public Object doInHibernate(Session se) throws HibernateException,
							SQLException {
						// TODO Auto-generated method stub
						se.createQuery("delete from TbCsMeter where id = '" + id + "'").executeUpdate();
						return null;
					}
				}
		);
